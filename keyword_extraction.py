...
def extract_keyphrases_survey(filename, nb_kp, min_char_length, max_words_length, min_words_length, min_keyword_frequency, groupby, headers, tradeoff):
    ...
    keyphrases = {qst:[(kw, "%.4f" % ((ct[0]-scaler_dic[qst]['min'])/scaler_dic[qst]['delta']), "%.5f" % ct[1]) for (kw,ct) in lst] for (qst,lst) in keyphrases.iteritems()}    

    if nb_kp > 0.99:
        nb_kp = int(nb_kp)
        keyphrases = {qst: lst[:nb_kp] for (qst, lst) in keyphrases.iteritems()}
    else:
        keyphrases = {qst: [(kw, ct, fr) for (kw, ct, fr) in lst if float(ct) > nb_kp] for (qst, lst) in keyphrases.iteritems()}
...
def extract_keyphrases_reviews(filename, nb_kp, min_char_length, max_words_length, min_words_length, min_keyword_frequency, groupby, headers, tradeoff):
    ...
    for col in columns_to_extract:
        ...
        if nb_kp > 0.99:
            ...
            keyphrases = {p_id: [(kw, ct, fr) for (kw, ct, fr) in lst if float(ct) > nb_kp] for (p_id, lst) in keyphrases.iteritems()}
